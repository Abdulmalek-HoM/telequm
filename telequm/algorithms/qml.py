...
try:
    from qiskit_machine_learning.algorithms import VQC, NeuralNetworkClassifier  # noqa: F401
...
class QuantumBeamformer:
    ...
    def compute_weights(
        ...
        fm_bound = self.feature_map.assign_parameters(
            dict(zip(self.feature_map.parameters, features, strict=False))
        )
        qc.compose(fm_bound, inplace=True)

        # Bind ansatz parameters
        ansatz_bound = self.ansatz.assign_parameters(
            dict(zip(self.ansatz.parameters, params, strict=False))
        )
    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            for channel, target in zip(channel_data, optimal_weights, strict=False):
                # Forward pass
